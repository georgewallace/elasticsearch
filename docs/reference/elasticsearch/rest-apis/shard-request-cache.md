...
You can control the size and expiration of the cache at the node level using the [shard request cache settings](shard-request-cache-settings.md).

::::{important} 
By default, the requests cache will only cache the results of search requests where `size=0`, so it will not cache `hits`, but it will cache `hits.total`,  [aggregations](search-aggregations.md), and [suggestions](search-suggesters.md).

Most queries that use `now` (see [Date Math](common-options.md#date-math)) cannot be cached.
...
The cache can be expired manually with the [`clear-cache` API](indices-clearcache.md):
...
It can also be enabled or disabled dynamically on an existing index with the [`update-settings`](indices-update-settings.md) API:
...
The size of the cache (in bytes) and the number of evictions can be viewed by index, with the [`indices-stats`](indices-stats.md) API:

```console
GET /_stats/request_cache?human
```

or by node with the [`nodes-stats`](cluster-nodes-stats.md) API:
