...
Matches [`geo_point`](geo-point.md) and [`geo_shape`](geo-shape.md) values within a given distance of a geopoint.
...
`distance`
:   The radius of the circle centred on the specified location. Points which fall into this circle are considered to be matches. The `distance` can be specified in various units. See [Distance Units](api-conventions.md#distance-units).
