...
By default, Elasticsearch SQL doesn’t search [frozen indices](frozen-indices.md). To search frozen indices, use one of the following features:

dedicated configuration parameter
:   Set to `true` properties `index_include_frozen` in the [SQL search API](sql-search-api.md) or `index.include.frozen` in the drivers to include frozen indices.
