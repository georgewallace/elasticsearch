...
Having multiple groups is fine, but when combined with `operator` or `minimum_should_match`, it can suffer from the [same problem](query-dsl-multi-match-query.md#operator-min) as `most_fields` or `best_fields`.
